    ...
    <div class="container m-t-1">
      ...
      <div class="row">
       ...
       <span class="btn-group" *ngIf="isAdmin()">
            <button type="button" class="btn btn-lg btn-danger" aria-label="Create message" [routerLink]="['/messages/create']"
                    routerLinkActive="active" title="{{'tooltip.news.add' | translate}}">
      ...
      <div class="row m-t-1">
        <div class="col-md-12">
          <div class="box" *ngIf="newsPage?.totalPages == 0">
            <p>{{"text.messages.recent.empty" | translate}}</p>
          </div>
          ...
          <div *ngIf="!loading">
            ...
            <div class="text-center m-b-1">
              <button (click)="toAllMessages()" class="btn btn-default">{{"text.messages.older" | translate}}</button>
            </div>
          </div>
        </div>
      </div>
...
export class RecentMessagesComponent implements OnInit {
  ...
  constructor(private _router: Router, private _api: NewsRestControllerService, private _errorHandler: ErrorHandlerService) {
  }
  ...
  getPage(page: number) {
    ...
    this._api.getLatestNews(page, this.searchTerm, 100)
        .subscribe(
            n => {
              this.newsPage = n;
              this.loading = false;
